...
- [~] Visitante anônimo usa landing, login demo, chat, feedback e `/insights` por HTTPS. *(P0.2 em 2026-08-11 confirmou landing e `/health` públicos, mas o login demo retornou `Erro de autenticação`; P0.3 encontrou cold start HTTP 504 em 81,72 s e exit 137 apenas em revisões antigas. A correção de timeout está testada localmente, mas a publicação está bloqueada por Docker Desktop parado e ACR Tasks indisponível; chat, feedback e `/insights` não puderam ser aceitos.)*
