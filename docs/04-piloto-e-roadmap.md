...
- [x] Visitante anônimo usa landing, login demo, chat, feedback e `/insights` por HTTPS. *(validado em 11/08/2026 na revisão pública final: landing, login após escala a zero (~95 s), chat RAG composto com fontes/agentes, feedback e `/insights`. A revisão atual da API não registrou exit 137; cold start configurado em até 180 s.)*
