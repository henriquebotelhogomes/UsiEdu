...
- [x] **Implementação do LLM-as-a-Judge Estruturado (2026-09-01):** Novo módulo `src/evaluation/llm_judge.py` com a classe `LLMJudge`, integrando rubricas semânticas (`faithfulness`, `context_precision`, `context_recall`, `answer_relevancy` e `rationale`) com o modelo `deepseek-v4-flash` via OpenCode Go. Harness `run_ragas.py` atualizado para utilizar o juiz semântico automaticamente quando online, preservando a flag `--heuristic` para testes offline. Testes unitários dedicados em `tests/unit/test_llm_judge.py` (538 passed total).
