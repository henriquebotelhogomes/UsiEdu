...
| Juiz | Manter provisoriamente `deepseek-v4-flash` como juiz econômico. Na comparação de 17 subperguntas × 3 repetições, `kimi-k2.7-code` atribuiu scores maiores em faithfulness e answer relevancy, mas custou mais e apresentou maior amplitude nessas métricas. Sem calibração humana, score maior não prova maior correção e não justifica a troca. | Rever após calibração humana amostral, mudança de preço/modelo ou novo protocolo. | 02: evidência completa em `src/evaluation/judge_comparison/2026-08-11/`; custo equivalente estimado US$ 0,319520. |
| Juiz — protocolo | Comparar no mesmo dataset, snapshot de manifest, recorte, respostas, prompt e métricas; executar três repetições; solicitar temperatura 0 quando suportada; registrar parâmetros, uso, divergência, estabilidade e custo estimado. O Console Go rejeitou temperatura 0 para ambos os modelos e exigiu temperatura efetiva 1, limitação preservada na configuração e no relatório. | Rever se o provedor passar a aceitar temperatura 0 ou após calibração humana. | 02: T02.5 concluída dentro do teto de US$ 5, sem criar ou registrar segredo. |
...
|---|---|
| Calibração humana dos juízes | Antes de afirmar que scores maiores de um juiz representam maior correção ou trocar o juiz econômico por esse motivo. |
