...
- [~] **T03.4 — Criar pipeline de promoção**
  ...
  - [~] Evidência: a execução hospedada bloqueou a promoção antes do push; o build registrou `msgpack 1.2.1` e `setuptools 84.0.0`, enquanto o relatório Trivy listou versões antigas. A nova execução usa Trivy `v0.73.0` e análise efêmera para confirmar o relatório da imagem recém-construída.
