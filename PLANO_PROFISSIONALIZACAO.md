...
- [x] Validar, na URL pública e após o ajuste de 2 vCPUs/4 GiB, o fluxo
  landing → login demo → chat com RAG → feedback → `/insights`. *(validado em
  11/08/2026; evidência em `docs/profissionalizacao/01-validacao-piloto.md`.)*
- [x] Registrar evidência de que uma pergunta documental responde sem o
  encerramento do contêiner com código 137. *(a revisão atual da API registrou
  zero eventos exit 137.)*
- [x] Medir e documentar tempo de cold start, primeira resposta e resposta com
  modelos já aquecidos. *(login frio observado em ~95 s; health aquecido em
  45–212 ms; timeout público configurado em 180 s.)*
- [x] Verificar `/health`, logs da API e execução bem-sucedida do job de
  ingestão após um novo deploy. *(job concluído e logs validados.)*
